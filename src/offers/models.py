...
class Offer(models.Model):
    ...
    price = models.DecimalField(max_digits=10, decimal_places=2, validators=[MinValueValidator(0.1)])
    buy_or_sell = models.CharField(max_length=10, choices=BUY_OR_SELL)
    is_active = models.BooleanField(default=True)


class Trade(models.Model):
    customer = models.ForeignKey(User, null=False, on_delete=models.CASCADE, related_name='customer')
    seller = models.ForeignKey(User, null=False, on_delete=models.CASCADE, related_name='seller')
    item = models.ForeignKey(Item, on_delete=models.CASCADE)
    number = models.PositiveIntegerField(null=False, validators=[MinValueValidator(1)])
